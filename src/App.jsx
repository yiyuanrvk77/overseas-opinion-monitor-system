...
function statusLabel(status) {
  return ({
    ...
    MACHINE_COUNT: '机器候选', VERIFIED_COUNT: '正式结论', EXCLUDED_COUNT: '未纳入正式结论',
    ...
    <div className="page-stack overview-page">
      ...
      <div className="overview-work-grid">
        ...
        <section className="panel coverage-panel">
          ...
          <div className="capability-boundary"><Info size={16} /><span>翻译能力尚未配置；情感、立场和风险须进入分析研判运行，机器候选经人工确认后方可作为正式结论。</span></div>
...
function AnalysisCandidate({ analysis, onReview }) {
  ...
  const workflow = field('workflow', 'status') || 'MACHINE_CANDIDATE'
  const reviewable = workflow === 'PENDING_HUMAN_REVIEW' || workflow === 'MACHINE_CANDIDATE'
  return <article className="analysis-candidate"><div className="analysis-candidate-head"><div><Badge tone={statusTone(field('workflow', 'status'))}>{statusLabel(field('workflow', 'status', 'MACHINE_CANDIDATE'))}</Badge><strong>{field('record_title', 'title') || '未命名记录'}</strong><small>{field('provider_label', 'model', 'engine') || '机器引擎未标注'}</small></div><Button variant="secondary" disabled={!reviewable} title={reviewable ? '进入人工研判' : '该候选已完成人工研判'} onClick={onReview}>{reviewable ? '人工研判' : '已完成研判'}</Button></div><div className="analysis-facts"><span>情感：{field('machine_sentiment', 'sentiment') || '未标注'}</span><span>立场：{field('machine_stance', 'stance') || '未标注'}</span><span>风险：{field('machine_risk_level', 'risk_level', 'risk') || '未标注'}</span><span>置信度：{field('confidence', 'machine_confidence') ?? '未标注'}</span></div><p>{field('machine_reason', 'narrative', 'reason', 'rationale') || '未提供机器判定理由。'}</p>{evidence && <div className="evidence-snippet"><FileCheck2 size={14} />{evidence}</div>}</article>
}

function HumanReviewDrawer({ analysis, role, onClose, onSaved }) {
  useEscapeClose(onClose)
  const [decision, setDecision] = useState('HUMAN_CONFIRMED'); const [note, setNote] = useState(''); const [sentiment, setSentiment] = useState(analysis.machine_sentiment || analysis.sentiment || ''); const [stance, setStance] = useState(analysis.machine_stance || analysis.stance || ''); const [risk, setRisk] = useState(analysis.machine_risk_level || analysis.risk_level || analysis.risk || ''); const [summary, setSummary] = useState(''); const [evidenceRefs, setEvidenceRefs] = useState((analysis.evidence_refs || []).join('\n')); const [saving, setSaving] = useState(false); const [error, setError] = useState('')
  const needsRevision = decision === 'HUMAN_REVISED'
  const submit = async (event) => {
    event.preventDefault(); if (!note.trim()) { setError('请填写人工研判备注，说明证据与判断依据。'); return }
    setSaving(true); setError('')
    try { const payload = await api.patch(`/api/analysis/${encodeURIComponent(analysis.id || analysis.analysis_id)}/review`, { role, decision, review_note: note.trim(), ...(needsRevision ? { human_sentiment: sentiment.trim(), human_stance: stance.trim(), human_risk_level: risk.trim(), human_summary: summary.trim(), evidence_refs: evidenceRefs.split(/\r?\n/).map((item) => item.trim()).filter(Boolean) } : {}) }); onSaved(payload) } catch (submitError) { setError(submitError.message || '提交人工研判失败') } finally { setSaving(false) }
  }
  return <div className="drawer-layer" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><aside className="drawer wide" role="dialog" aria-modal="true" aria-label="人工研判"><div className="drawer-header"><div><span>机器候选 · {analysis.id || analysis.analysis_id}</span><h2>人工研判</h2></div><button className="icon-btn" onClick={onClose} aria-label="关闭人工研判"><X size={19} /></button></div><form className="drawer-body action-form" onSubmit={submit}><div className="action-subject"><Badge>{analysis.provider_label || analysis.model || '机器判定'}</Badge><strong>{analysis.record?.title || analysis.record_title || '待研判记录'}</strong><p>{analysis.machine_reason || analysis.narrative || analysis.reason || '请基于原始记录和证据片段完成研判。'}</p></div><label className="form-field"><span>研判决定</span><select value={decision} onChange={(event) => setDecision(event.target.value)} disabled={role !== 'core'}><option value="HUMAN_CONFIRMED">确认机器判定</option><option value="HUMAN_REVISED">修订机器判定</option><option value="HUMAN_REJECTED">驳回机器判定</option><option value="NEEDS_MORE_EVIDENCE">补充证据</option></select></label>{needsRevision && <><label className="form-field"><span>人工情感</span><input value={sentiment} onChange={(event) => setSentiment(event.target.value)} placeholder="例如：正面 / 中性 / 负面" /></label><label className="form-field"><span>人工立场</span><input value={stance} onChange={(event) => setStance(event.target.value)} placeholder="例如：支持 / 中立 / 反对" /></label><label className="form-field"><span>人工风险等级</span><input value={risk} onChange={(event) => setRisk(event.target.value)} placeholder="例如：低 / 中 / 高" /></label><label className="form-field"><span>人工摘要</span><textarea rows="4" value={summary} onChange={(event) => setSummary(event.target.value)} placeholder="概括人工修订后的判断" /></label><label className="form-field"><span>证据引用（每行一条）</span><textarea rows="4" value={evidenceRefs} onChange={(event) => setEvidenceRefs(event.target.value)} placeholder="填写原始链接或证据编号，每行一条" /></label></>}<label className="form-field"><span>人工研判备注（必填）</span><textarea required rows="6" value={note} onChange={(event) => setNote(event.target.value)} placeholder="说明依据、判断过程及需要跟进的事项" /></label>{error && <Notice tone="danger">{error}</Notice>}<div className="form-actions"><Button variant="secondary" onClick={onClose}>取消</Button><Button type="submit" icon={CheckCircle2} disabled={role !== 'core' || saving}>{saving ? '提交中…' : '提交人工研判'}</Button></div></form></aside></div>
}
